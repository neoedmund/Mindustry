...
public class PlanetDialog extends BaseDialog implements PlanetInterfaceRenderer {
	static final String[] defaultIcons = { "effect", "power", "logic", "units", "liquid", "production", "defense",
			"turret", "distribution", "crafting", "settings", "cancel", "zoom", "ok", "star", "home", "pencil", "up",
			"down", "left", "right", "hammer", "warning", "tree", "admin", "map", "modePvp", "terrain", "modeSurvival",
	...
	public PlanetDialog() {
		...
		addListener(new InputListener() {
			...
			public boolean keyDown(InputEvent event, KeyCode key) {
				if (event.targetActor == PlanetDialog.this && (key == KeyCode.escape || key == KeyCode.back
	...
	void setup() {
		...
		stack(new Element() {
			...
			public void act(float delta) {
				if (scene.getDialog() == PlanetDialog.this && !scene.hit(input.mouseX(), input.mouseY(), true)
							...
							for (int i = 0; i < content.planets().size; i++) {
								...
								if (selectable(planet)) {
									pt.button(planet.localizedName,
											...
											.update(bb -> bb.setChecked(state.planet == planet))
											.with(w -> w.marginLeft(10f)).get().getChildren().get(1)
	...
	void rebuildExpand() {
		...
		if (state.planet.sectors.contains(Sector::hasBase)) {
			...
			c.button(
					...
					Icon.downOpen, Styles.squareTogglet, () -> sectorsShown = !sectorsShown).height(60f).checked(b -> {
	...
	public void lookAt(Sector sector, float alpha) {
		...
		state.camPos.slerp(Tmp.v31.set(sector.tile.v).rotate(Vec3.Y, -sector.planet.getRotation()).setLength(len),
				alpha);
	}
	...
	public void act(float delta) {
		...
		if (hovered != null && !mobile && state.planet.hasGrid()) {
			...
			Vec3 pos = planets.cam.project(Tmp.v31.set(hovered.tile.v).setLength(PlanetRenderer.outlineRad)
		...
		if (state.planet.hasGrid()) {
			hovered = Core.scene.getDialog() == this
					? state.planet.getSector(planets.cam.getMouseRay(), PlanetRenderer.outlineRad)
					: null;
