...
public class Damage {
	...
	public static void applySuppression(Team team, float x, float y, float range, float reload, float maxDelay,
			float applyParticleChance, @Nullable Position source) {
		builds.clear();
		indexer.eachBlock(null, x, y, range, build -> build.team != team, build -> {
			...
			if (build.wasRecentlyHealed(60f * 12f) || build.block.suppressable) {
				...
				// add prev check so ability spam doesn't lead to particle spam (essentially,
				// recently suppressed blocks don't get new particles)
		...
		// to prevent particle spam, the amount of particles is to remain constant
		...
		for (var build : builds) {
			if (Mathf.chance(scaledChance)) {
				Time.run(Mathf.random(maxDelay), () -> {
					Fx.regenSuppressSeek.at(build.x + Mathf.range(build.block.size * tilesize / 2f),
							build.y + Mathf.range(build.block.size * tilesize / 2f), 0f, source);
				});
			}
		}
	}
	...
	public static void dynamicExplosion(float x, float y, float flammability, float explosiveness, float power,
			float radius, boolean damage) {
		dynamicExplosion(x, y, flammability, explosiveness, power, radius, damage, true, null, Fx.dynamicExplosion);
	}

	/** Creates a dynamic explosion based on specified parameters. */
	public static void dynamicExplosion(float x, float y, float flammability, float explosiveness, float power,
			float radius, boolean damage, Effect explosionFx) {
		dynamicExplosion(x, y, flammability, explosiveness, power, radius, damage, true, null, explosionFx);
	}

	/** Creates a dynamic explosion based on specified parameters. */
	public static void dynamicExplosion(float x, float y, float flammability, float explosiveness, float power,
			float radius, boolean damage, boolean fire, @Nullable Team ignoreTeam) {
		dynamicExplosion(x, y, flammability, explosiveness, power, radius, damage, fire, ignoreTeam,
				Fx.dynamicExplosion);
	}

	/** Creates a dynamic explosion based on specified parameters. */
	public static void dynamicExplosion(float x, float y, float flammability, float explosiveness, float power,
			float radius, boolean damage, boolean fire, @Nullable Team ignoreTeam, Effect explosionFx) {
		if (damage) {
			for (int i = 0; i < Mathf.clamp(power / 700, 0, 8); i++) {
				int length = 5 + Mathf.clamp((int) (Mathf.pow(power, 0.98f) / 500), 1, 18);
				Time.run(i * 0.8f + Mathf.random(4f), () -> Lightning.create(Team.derelict, Pal.power,
						3 + Mathf.pow(power, 0.35f), x, y, Mathf.random(360f), length + Mathf.range(2)));
			}

			if (fire) {
				for (int i = 0; i < Mathf.clamp(flammability / 4, 0, 30); i++) {
					Time.run(i / 2f, () -> Call.createBullet(Bullets.fireball, Team.derelict, x, y, Mathf.random(360f),
							Bullets.fireball.damage, 1, 1));
				}
			}
			...
			for (int i = 0; i < waves; i++) {
				int f = i;
				Time.run(i * 2f, () -> {
					damage(ignoreTeam, x, y, Mathf.clamp(radius + explosiveness, 0, 50f) * ((f + 1f) / waves),
							explosiveness / 2f, false);
	...
	public static @Nullable Building findAbsorber(Team team, float x1, float y1, float x2, float y2) {
		...
		boolean found = World.raycast(World.toTile(x1), World.toTile(y1), World.toTile(x2), World.toTile(y2),
				(x, y) -> (tmpBuilding = world.build(x, y)) != null && tmpBuilding.team != team
						&& tmpBuilding.block.absorbLasers);
	...
	public static float findPierceLength(Bullet b, int pierceCap, float length) {
		...
		Units.nearbyEnemies(b.team, rect, u -> {
			...
			if (u.checkTarget(b.type.collidesAir, b.type.collidesGround) && u.hittable()
					&& Intersector.intersectSegmentRectangle(b.x, b.y, b.x + vec.x, b.y + vec.y, hitrect)) {
		...
		return Math.min(
				distances.size < pierceCap || pierceCap < 0 ? length : Math.max(6f, distances.get(pierceCap - 1)),
				maxDst);
	}

	/**
	 * TODO Collides a bullet with blocks in a laser, taking into account absorption
	...
	public static void collideLine(Bullet hitter, Team team, Effect effect, float x, float y, float angle,
			float length) {
	 ...
	 */
	public static void collideLine(Bullet hitter, Team team, Effect effect, float x, float y, float angle, float length,
			boolean large, boolean laser, int pierceCap) {
		if (laser) {
		...
		if (hitter.type.collidesGround) {
			...
			World.raycastEachWorld(x, y, seg2.x, seg2.y, (cx, cy) -> {
				...
				for (Point2 p : Geometry.d4) {
					...
					if (other != null
							&& (large || Intersector.intersectSegmentRectangle(seg1, seg2, other.getBounds(Tmp.r1)))) {
		...
		Cons<Unit> cons = e -> {
			if (!e.hittable())
				return;
			// the peirce cap works for units, but really terribly, I'm just disabling it
			// for now.
	...
	public static Healthc linecast(Bullet hitter, float x, float y, float angle, float length) {
		...
		Units.nearbyEnemies(hitter.team, rect, e -> {
			if ((tmpUnit != null && e.dst2(x, y) > tmpUnit.dst2(x, y))
					|| !e.checkTarget(hitter.type.collidesAir, hitter.type.collidesGround)
					|| !e.targetable(hitter.team))
				return;
		...
		if (tmpBuilding != null && tmpUnit != null) {
			if (Mathf.dst2(x, y, tmpBuilding.getX(), tmpBuilding.getY()) <= Mathf.dst2(x, y, tmpUnit.getX(),
					tmpUnit.getY())) {
				return tmpBuilding;
			}
	...
	public static void damageUnits(Team team, float x, float y, float size, float damage, Boolf<Unit> predicate,
			Cons<Unit> acceptor) {
		Cons<Unit> cons = entity -> {
			if (!predicate.get(entity) || !entity.hittable())
				return;
	...
	public static void status(Team team, float x, float y, float radius, StatusEffect effect, float duration,
			boolean air, boolean ground) {
		Cons<Unit> cons = entity -> {
			if (entity.team == team || !entity.checkTarget(air, ground) || !entity.hittable()
					|| !entity.within(x, y, radius)) {
				return;
			}
	...
	/** Damages all entities and blocks in a radius that are enemies of the team. */
	public static void damage(Team team, float x, float y, float radius, float damage, boolean complete, boolean air,
			boolean ground) {
	...
	public static void tileDamage(Team team, int x, int y, float baseRadius, float damage, @Nullable Bullet source) {
		...
		Core.app.post(() -> {
			...
			// spawned inside a multiblock. this means that damage needs to be dealt
			// directly.
			// why? because otherwise the building would absorb everything in one cell,
			// which means much less damage than a nearby explosion.
			// this needs to be compensated
			if (in != null && in.team != team && in.block.size > 1 && in.health > damage) {
				// deal the damage of an entire side, to be equivalent with maximum 'standard'
				// damage
			...
			for (int i = 0; i <= rays; i++) {
				...
				int startX = x;
				int startY = y;
				int endX = x + (int) (Math.cos(spacing * i) * radius),
						endY = y + (int) (Math.sin(spacing * i) * radius);
	...
	private static void completeDamage(Team team, float x, float y, float radius, float damage) {
		...
		for (int dx = -trad; dx <= trad; dx++) {
			for (int dy = -trad; dy <= trad; dy++) {
				...
				if (tile != null && tile.build != null && (team == null || team.isEnemy(tile.team()))
						&& dx * dx + dy * dy <= trad * trad) {
