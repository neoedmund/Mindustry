...
public class BulletType extends Content implements Cloneable {
	...
	/**
	 * If false, this bullet isn't removed after pierceCap is exceeded. Expert usage
	 * only.
	 */
	public boolean removeAfterPierce = true;
	/**
	 * For piercing lasers, setting this to true makes it get absorbed by plastanium
	 * walls.
	 */
	public boolean laserAbsorb = true;
	/**
	 * Life fraction at which this bullet has the best range/damage/etc. Used for
	 * lasers and continuous turrets.
	 */
	...
	/**
	 * Effect created when charging starts; only usable in single-shot weapons with
	 * a firstShotDelay / shotDelay.
	 */
	...
	/**
	 * If true, splash damage is "correctly" affected by unit hitbox size. Used for
	 * projectiles that do not collide / have splash as their main source of damage.
	 */
	...
	/**
	 * Whether to scale lifetime (not actually velocity!) to disappear at the target
	 * position. Used for artillery.
	 */
	...
	/**
	 * Whether to move the bullet back depending on delta to fix some delta-time
	 * related issues. Do not change unless you know what you're doing.
	 */
	...
	/**
	 * When used in a turret with multiple ammo types, this can be set to a non-zero
	 * value to influence range.
	 */
	...
	/**
	 * Whether to create hit effects on despawn. Forced to true if this bullet has
	 * any special effects like splash damage.
	 */
	public boolean despawnHit = false;
	/**
	 * If true, this bullet will create bullets when it hits anything, not just when
	 * it despawns.
	 */
	public boolean fragOnHit = true;
	/**
	 * If true, unit armor is ignored in damage calculations. Ignored for building
	 * armor.
	 */
	...
	/**
	 * Bullets spawned when this bullet is created. Rarely necessary, used for
	 * visuals.
	 */
	...
	/**
	 * Unit spawned when this bullet hits something or despawns due to it hitting
	...
	/**
	 * Whether the parameter passed to the trail is the bullet rotation, instead of
	 * a flat value.
	 */
	...
	/**
	 * Power of bullet ability. Usually a number between 0 and 1; try 0.1 as a
	 * starting point.
	 */
	...
	/**
	 * Chance of suppression effect occurring on block, scaled down by number of
	 * blocks.
	 */
	...
	protected float calculateRange() {
		...
		if (spawnUnit != null)
			return spawnUnit.lifetime * spawnUnit.speed;
		return Math.max(Mathf.zero(drag) ? speed * lifetime : speed * (1f - Mathf.pow(1f - drag, lifetime)) / drag,
				maxRange);
	}
	...
	public void hitEntity(Bullet b, Hitboxc entity, float health) {
		...
		Unit unit) {
			...
			if (impact)
				Tmp.v3.setAngle(b.rotation() + (knockback < 0 ? 180f : 0f));
			unit.impulse(Tmp.v3);
			unit.apply(status, statusDuration);
		}

		// for achievements
		if (b.owner instanceof WallBuild && player != null && b.team == player.team() && entity instanceof Unit unit
				&& unit.dead) {
	...
	public float damageMultiplier(Bullet b) {
		if (b.owner instanceof Unit u)
			return u.damageMultiplier() * state.rules.unitDamage(b.team);
		if (b.owner instanceof Building)
			return state.rules.blockDamage(b.team);
	...
	public void createSplashDamage(Bullet b, float x, float y) {
		if (splashDamageRadius > 0 && !b.absorbed) {
			Damage.damage(b.team, x, y, splashDamageRadius, splashDamage * b.damageMultiplier(), false, collidesAir,
					collidesGround, scaledSplashDamage, b);
			...
			if (makeFire) {
				indexer.eachBlock(null, x, y, splashDamageRadius, other -> other.team != b.team,
		...
		if (fragBullet != null) {
			for (int i = 0; i < fragBullets; i++) {
				...
				float a = b.rotation() + Mathf.range(fragRandomSpread / 2) + fragAngle
						+ ((i - fragBullets / 2) * fragSpread);
				fragBullet.create(b, x + Angles.trnsx(a, len), y + Angles.trnsy(a, len), a,
	...
	/**
	 * Called when the bullet reaches the end of its lifetime or is destroyed by
	 * something external.
	 */
	...
	public void updateBulletInterval(Bullet b) {
		if (intervalBullet != null && b.timer.get(2, bulletInterval)) {
			...
			for (int i = 0; i < intervalBullets; i++) {
				intervalBullet.create(b, b.x, b.y, ang + Mathf.range(intervalRandomSpread) + intervalAngle
	...
	public void updateWeaving(Bullet b) {
		if (weaveMag != 0) {
			b.vel.rotateRadExact((float) Math.sin((b.time + Math.PI * weaveScale / 2f) / weaveScale) * weaveMag
	...
	public void updateTrail(Bullet b) {
		if (!headless && trailLength > 0) {
			...
			b.trail.update(b.x, b.y, trailInterp.apply(b.fin())
					* (1f + (trailSinMag > 0 ? Mathf.absin(Time.time, trailSinScl, trailSinMag) : 0f)));
		}
	}
	...
	public void init() {
		if (pierceCap >= 1) {
			pierce = true;
			// pierceBuilding is not enabled by default, because a bullet may want to *not*
			// pierce buildings
		}
	...
	public @Nullable Bullet create(@Nullable Entityc owner, Team team, float x, float y, float angle, float damage,
			float velocityScl, float lifetimeScl, Object data) {
		return create(owner, team, x, y, angle, damage, velocityScl, lifetimeScl, data, null);
	}

	public @Nullable Bullet create(@Nullable Entityc owner, Team team, float x, float y, float angle, float damage,
			float velocityScl, float lifetimeScl, Object data, @Nullable Mover mover) {
	...
	public void createNet(Team team, float x, float y, float angle, float damage, float velocityScl,
			float lifetimeScl) {
		Call.createBullet(this, team, x, y, angle, damage, velocityScl, lifetimeScl);
	}

	@Remote(called = Loc.server, unreliable = true)
	public static void createBullet(BulletType type, Team team, float x, float y, float angle, float damage,
			float velocityScl, float lifetimeScl) {
		if (type == null)
			return;
