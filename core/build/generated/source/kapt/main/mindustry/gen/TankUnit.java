...
public class TankUnit extends Unit implements Boundedc, Builderc, Drawc, ElevationMovec, Entityc, Flyingc, Healthc, Hitboxc, Itemsc, Minerc, Physicsc, Posc, Rotc, Shieldc, Statusc, Syncc, Tankc, Teamc, Unitc, Velc, Weaponsc {
  ...
  public void destroy() {
        ...
        if (!spawnedByCore) {
            Damage.dynamicExplosion(x, y, flammability, explosiveness, power, (bounds() + type.legLength / 1.7F) / 2.0F, state.rules.damageExplosions, item().flammability > 1, team, type.deathExplosionEffect);
        } else {
    ...
    tank: {
        ...
        for (int dx = -r; dx <= r; dx++) {
            for (int dy = -r; dy <= r; dy++) {
                ...
                if (type.crushDamage > 0 && walked && t != null && t.build != null && t.build.team != team && Math.max(Math.abs(dx), Math.abs(dy)) <= r - 1) {
